...
class AutomatorServer(object):
    ...
    def sdk_version(self):
        ...
        if self.__sdk == 0:
            try:
                ...
                # os.popen('adb shell getprop ro.build.version.sdk ').read())
            except:
                pass
        return self.__sdk
    ...
    def start(self, timeout=5):
        ...
        for loop in range(10):
            if self.sdk_version() > 0:
                print "self.sdk_version:", self.sdk_version()
                break
            time.sleep(5)
    ...
    def getLocalport(self, deviceid):
        ...
        try:
            ...
            print "get the operation system failed"
            print traceback.format_exc()
...
class AutomatorDeviceUiObject(object):
    ...
    def __getattr__(self, attr):
        '''alias of fields in info property.'''
        info = self.info
        print "__getattr__ info is--------->",info
        if attr in info:
            return info[attr]
        elif attr in self.__alias:
            return info[self.__alias[attr]]
    ...
    def ext10(self):
        ...
        '''
        method = self.device.server.jsonrpc_wrap(timeout=80).waitForExists
        return method(self.selector, 10000)
